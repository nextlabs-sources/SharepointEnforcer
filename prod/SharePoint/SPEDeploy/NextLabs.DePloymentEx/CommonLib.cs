...
namespace NextLabs.Deployment
{
    ...
    public class CommonLib
    {
        ...
        public const string xpathRoot = "/configuration/system.webServer/modules";

        public const string httpModuleName = "NextLabs.HttpEnforcer.HttpEnforcerModule";

        public const string httpModuleType = "NextLabs.HttpEnforcer.HttpEnforcerModule, NextLabs.SPEnforcer, Version=3.0.0.0, Culture=neutral, PublicKeyToken=5ef8e9c15bdfa43e";
        ...
        const string NEXTLABSFEATUREFILE = "C:\\Program Files\\Common Files\\Microsoft Shared\\Web Server Extensions\\16\\TEMPLATE\\FEATURES\\NextLabs.DePloyment_NextLabs.Entitlement.Basic\\Feature.xml";//bruce modify
        ...
        public bool IsSPEInstalledWithoutCEPCOnlyUseJPC()
        {
            bool useJpc = false;
            RegistryKey CE_key = null;
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                CE_key = Registry.LocalMachine.OpenSubKey("Software\\NextLabs\\Compliant Enterprise\\Sharepoint Enforcer\\", false);
                object JpcEnabled = null;
                if (CE_key != null)
                {
                    JpcEnabled = CE_key.GetValue("enableJPCwithoutCEPC");
                }
                if(JpcEnabled != null)
                {
                    useJpc = Convert.ToBoolean(JpcEnabled);
                }
            });
            return useJpc;
        }
        ...
        private int DuplicatesCount(string webappName, string webconfigPath)
        {
            int count = 0;
            if(File.Exists(webconfigPath))
            {
                string xpath = xpathRoot + "/add/@name";
                XmlDocument domWebConfig = new XmlDocument();
                domWebConfig.Load(webconfigPath);
                XmlNodeList nodes = domWebConfig.SelectNodes(xpath);
                foreach (XmlNode node in nodes)
                {
                    if (Equals(httpModuleName, node.Value))
                    {
                        count++;
                    }
                }
            ...
            {
                Progress.WriteLog(webappName, "DuplicatesCount, can not find web.config, path is:" + webconfigPath);
            }
            return count;
        }
        private void ModifyHttpEnforcerModuleToWebconfig(string webappName, string webconfigPath)
        { 
            if (File.Exists(webconfigPath))
            {
                try
                {
                    ...
                    XmlDocument domWebConfig = new XmlDocument();
                    domWebConfig.Load(webconfigPath);
                    XmlNode parentnode = domWebConfig.SelectSingleNode(xPath);
                    XmlElement newNode = domWebConfig.CreateElement("add");
                    newNode.SetAttribute("name", httpModuleName);
                    newNode.SetAttribute("type", httpModuleType);
                    parentnode.AppendChild(newNode);
                    domWebConfig.Save(webconfigPath);
                }
                catch(Exception exp)
                {
                    Progress.WriteLog(webappName, "ModifyHttpEnforcerModuleToWebconfig fails: " + exp);
                }

            }
            else
            {
                Progress.WriteLog(webappName, "ModifyHttpEnforcerModuleToWebconfig, can not find web.config, path is:" + webconfigPath);
            }
        }
        private void DeleteHttpEnforcerModuleFromWebconfig(string webappName, string webconfigPath)
        {
            if(File.Exists(webconfigPath))
            {
                try
                {
                    ...
                    XmlDocument domWebConfig = new XmlDocument();
                    domWebConfig.Load(webconfigPath);
                    XmlNode deleteKey = domWebConfig.SelectSingleNode((xPath.Replace("?", httpModuleName)));
                    deleteKey.ParentNode.RemoveChild(deleteKey);
                    domWebConfig.Save(webconfigPath);
                }
                catch(Exception exp)
                {
                    Progress.WriteLog(webappName, "DeleteHttpEnforcerModuleFromWebconfig fails: " + exp);
                }
            }
            else
            {
                Progress.WriteLog(webappName, "DeleteHttpEnforcerModuleFromWebconfig, can not find web.config, path is:" + webconfigPath);
            }
        }
        public bool GetInvalidFarmSPServers(ref List<string> servers, string webappName)
        {
            bool result = false;
            try
            {
                ...
                {
                    if(server.Role != SPServerRole.Invalid && server.Role != SPServerRole.Search)
                    {
            ...
            {
                Progress.WriteLog(webappName, "GetInvalidFarmSPServers fails: " + exp);
            }
            return result;
        }

        private  string FormatRemoteFilePath(string oldPath)
        {
        ...
        public string GetWebconfigFilePath(ref SPWebApplication webapp)
        {
            string webconfigFilePath = String.Empty;
            try
            {
                SPUrlZone zone = webapp.AlternateUrls[0].UrlZone;
                SPIisSettings iisSettings = webapp.IisSettings[zone];
                webconfigFilePath = iisSettings.Path.ToString() + "\\web.config";
            }
            catch(Exception exp)
            {
                Progress.WriteLog(webapp.Name, "GetWebconfigFilePath fails: " + exp);
            }
            return webconfigFilePath;
        }

        public void EditWebconfig(string server, Operations action, string path, string webappName)
        {
            if(string.IsNullOrEmpty(path))
            {
                Progress.WriteLog(webappName, $"EditWebconfig get wrong format web.config path: " + path);
                return;
            }
            string formatPath = FormatRemoteFilePath(path);
            string webconfigPath = $"\\\\{server}\\{formatPath}";//in hz-ts, path is C:\programe files\NextLabs\web.config, remote path format is \\hz-ts\C$\programe files\NextLabs\web.config
            try
            {
                switch (action)
                {
                    ...
                    {
                        if(DuplicatesCount(webappName, webconfigPath) > 0)
                        {
                            DeleteHttpEnforcerModuleFromWebconfig(webappName, webconfigPath);
                        }
                        break;
                    }
                    case Operations.Edit:
                    {
                        if(DuplicatesCount(webappName, webconfigPath) < 1)
                        {
                            ModifyHttpEnforcerModuleToWebconfig(webappName, webconfigPath);
                        }
                        break;
                    }
                }
            ...
            {
                Progress.WriteLog(webappName, "EditWebconfig fails: " + exp);
            }
        }
