...
print "update version for cpp projects, args count: $argCount\n";
if ($argCount != 7)
{
	dumpParameters;
	exit 1;
...
$buf =~ s#VALUE\s+"FileVersion"\s*,\s*"[^"]*"#VALUE "FileVersion", "$majorVer.$minorVer.$maintenanceVer.$patchVer"#g;
...
$buf =~ s#VALUE\s+"ProductVersion"\s*,\s*"[^"]*"#VALUE "ProductVersion", "$majorVer.$minorVer.$maintenanceVer.$patchVer"#g;
