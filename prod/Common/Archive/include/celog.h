  ...
  public:
     ...
     */
	 ...
	 return -1;
       ...
       if (sTmpTime)
           strncpy(timeline, sTmpTime, sizeof(timeline));
       if (sTmpTime)
       {
           timeline[strlen(timeline)-1] = (char)NULL;  /* remove newline */
           p += snprintf(p,_countof(sline), "%.19s.%03hu %s: ",timeline,timebuffer.millitm,&timeline[20]);
       }

       /* log level in string form */
       p += snprintf(p, _countof(sline) - (p-sline), "%s ", LevelToString(lvl));
#endif
       ...
       {
#if defined(_WIN32) || defined(_WIN64)
	 p += _snprintf_s(p, sizeof(sline), _countof(sline), "%s [%d]: ", file, line);
#else
	 p +=  snprintf(p, sizeof(sline), "%s [%d]: ", file, line);
#endif
       }
       ...
       va_end(args);
       sline[CELOG_MAX_MESSAGE_SIZE_CHARS - 1] = '\0';

#if defined(_WIN32) || defined (_WIN64)
       wchar_t wsline[CELOG_MAX_MESSAGE_SIZE_CHARS];
       size_t nchars = 0;
       errno_t err_val = mbstowcs_s(&nchars,wsline,_countof(wsline),sline,CELOG_MAX_MESSAGE_SIZE_CHARS);
       if( err_val != 0 )
       {
           return -1; /* fail */
       }
	 ...
	 return -1;
       ...
       {
           timeline[strlen(timeline)-1] = (char)NULL;  /* remove newline */
           p += swprintf(p,_countof(sline), L"%.19hs.%03hu %hs: ",timeline,timebuffer.millitm,&timeline[20]);
       }

       /* log level in string form */
       p += swprintf(p, _countof(sline) - (p-sline), L"%s ", LevelToStringW(lvl));

       /* File may be NULL to indicate there is not file/line information */
       if( file != NULL )
       {
	 p += swprintf(p, _countof(sline) - (p-sline), L"%hs [%d]: ", file, line);
       }

       va_list args;
       va_start(args,fmt);
       p += vswprintf(p,_countof(sline) - (p-sline),fmt,args);
       va_end(args);
