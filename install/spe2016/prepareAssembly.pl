...
my	$installDir = "$buildRootDir/install/spe2016";
...
my	$srcBinSP2016CsDir = "$buildRootDir/bin/release_dotnet/SP2016release";
...
my	@enforcerCommonbin64FileList = (
		["policy_engine.dll",$versionStr, "x64", $releaseOnly]
	);

my	@enforcerCommon3rdPartBin64FileList= (
		["boost_date_time-vc140-mt-x64-1_67.dl",$versionStr, "x64", $releaseOnly],
		["jsoncpp.dll",				        	$versionStr, "x64", $releaseOnly],
		["LIBEAY32.dll",						$versionStr, "x64", $releaseOnly],
		["SSLEAY32.dll",						$versionStr, "x64", $releaseOnly]
	);
...
InstallHelper::copyListVersionRequired($srcBinSP2016CsDir, $destBinCsDir, \@binSP2016CsFileList);
...
InstallHelper::copyRequired("$srcEnforcerCommonDir/thirdpart/x64-windows/release/bin/boost_date_time-vc140-mt-x64-1_67.dll", "$destBin64Dir/boost_date_time-vc140-mt-x64-1_67.dll");
InstallHelper::copyRequired("$srcEnforcerCommonDir/thirdpart/x64-windows/release/bin/jsoncpp.dll", "$destBin64Dir/jsoncpp.dll");
InstallHelper::copyRequired("$srcEnforcerCommonDir/thirdpart/x64-windows/release/bin/LIBEAY32.dll", "$destBin64Dir/LIBEAY32.dll");
InstallHelper::copyRequired("$srcEnforcerCommonDir/thirdpart/x64-windows/release/bin/SSLEAY32.dll", "$destBin64Dir/SSLEAY32.dll");
InstallHelper::copyRequired("$srcEnforcerCommonDir/bin/release_win_x64/policy_engine.dll", "$destBin64Dir/policy_engine.dll");
...
InstallHelper::copyVersionRequired("$externalDir/microsoft/sharepoint/sp2016/DocumentFormat.OpenXml.dll","$destBinCsDir/DocumentFormat.OpenXml.dll","2.5.5631.0");
#
...
my	$srcSpeConfigSp2016Dir = "$srcSpeConfigDir/sp2016";
my	$destResDir = "$assemblyDir/resource";
my	$destConfigDir = "$assemblyDir/configuration";
my	$destConfigSp2016Dir = "$assemblyDir/configuration/sp2016";

# Common
InstallHelper::copyRequired("$srcResDir/README.txt",									"$destResDir/README.txt");
InstallHelper::copyRequired("$srcBinSP2016CsDir/NextLabs.Entitlement.wsp",				"$destBinCsDir/NextLabs.Entitlement.wsp");
...
InstallHelper::copyRequired("$srcSpeConfigDir/sp2016/InstallBasicFeatureFor2016Mixed.ps1",		"$destBinCsDir/InstallBasicFeatureFor2016Mixed.ps1");
InstallHelper::copyRequired("$srcSpeConfigDir/sp2016/InstallEventReceiverFeatureFor2016Mixed.ps1",		"$destBinCsDir/InstallEventReceiverFeatureFor2016Mixed.ps1");
InstallHelper::copyRequired("$srcSpeConfigDir/sp2016/UninstallBasicFeatureFor2016Mixed.ps1",		"$destBinCsDir/UninstallBasicFeatureFor2016Mixed.ps1");
InstallHelper::copyRequired("$srcSpeConfigDir/sp2016/UninstallEventReceiverFeatureFor2016Mixed.ps1",		"$destBinCsDir/UninstallEventReceiverFeatureFor2016Mixed.ps1");
...
if (system("cp -fR \"$srcSpeConfigSp2016Dir\" \"$destConfigSp2016Dir\""))
{
	die "### ERROR: Failed to copy configuration files from $srcSpeConfigSp2016Dir to $destConfigSp2016Dir.\n";
}
...
# 64-bit
InstallHelper::copyRequired("$srcSpeConfigSp2016Dir/built-in components for PLE(2016).xml",		"$package64Dir/PLE.xml");
